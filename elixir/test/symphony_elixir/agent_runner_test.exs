defmodule SymphonyElixir.AgentRunnerTest do
  ...
  alias SymphonyElixir.AgentRunner.CodexRunner
  ...
  describe "Behavior" do
    test "format_token_report formats token counts correctly" do
      ...
      assert CodexRunner.format_token_report(tokens) == "tokens: 1500 total (1000 in / 500 out)"
    end

    test "format_token_report handles zero tokens" do
      tokens = %{input_tokens: 0, output_tokens: 0, total_tokens: 0}
      assert CodexRunner.format_token_report(tokens) == "tokens: 0 total (0 in / 0 out)"
    end
    ...
    test "format_token_report handles large token counts" do
      ...
      assert CodexRunner.format_token_report(tokens) ==
               "tokens: 2250000 total (1500000 in / 750000 out)"
    end
  end

  describe "CodexRunner" do
    @workspace_path System.tmp_dir!()
                    |> Path.join("symphony-codex-runner-test-#{:rand.uniform(999_999)}")
    ...
    test "workspace_path returns a valid path string" do
      ...
      assert @workspace_path =~ "symphony-codex-runner-test"
    end
  end
