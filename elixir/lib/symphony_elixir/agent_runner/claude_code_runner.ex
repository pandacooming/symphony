defmodule SymphonyElixir.AgentRunner.ClaudeCodeRunner do
  @moduledoc """
  AgentRunner implementation for Claude Code using the AppServer protocol.

  Wraps the Claude Code session and emits typed events for the orchestrator:
  - `turn_start` / `turn_end` for each turn
  ...
  def init({workspace_path, prompt, config}) do
    cc_config = Config.claude_code_config()
    ...
    issue = Map.get(config, :issue, %{id: "runner", identifier: "runner", title: "Claude Code Runner"})
  ...
  def handle_continue(:start_session, state) do
    case start_claude_code_session(state) do
      {:ok, port, session_id} ->
  ...
  @impl true
  def handle_info({:claude_code_message, message}, state) do
  ...
  defp start_claude_code_session(state) do
    workspace = state.workspace_path
    cc_config = state.cc_config

    # Build environment with API keys
    env = build_claude_code_env(cc_config)

    # Build command
    cmd = cc_config.command
    ...
    port =
      Port.open(
        ...
        [
          ...
          :exit_status,
          :stderr_to_stdout,
          args: [~c"-lc", "cd #{shell_escape(workspace)} && exec #{shell_escape(cmd)}"],
          env: env,
      ...
      {^port, {:data, {:eol, line}}} ->
        case Jason.decode(to_string(line)) do
          {:ok, %{"method" => "session/started", "params" => params}} ->
            session_id = params["sessionId"] || "claude-#{:rand.uniform(999_999)}"
          ...
          {:ok, payload} ->
            ...
            session_id = extract_session_id(payload) || "claude-#{:rand.uniform(999_999)}"
            {:ok, port, session_id}

          {:error, _} ->
            # Non-JSON response might mean ready to proceed
            {:ok, port, "claude-#{:rand.uniform(999_999)}"}
        end
  ...
  defp build_claude_code_env(cc_config) do
    env_list = [
      {"LINEAR_API_KEY", to_charlist(System.get_env("LINEAR_API_KEY") || "")},
      {"ANTHROPIC_API_KEY", to_charlist(cc_config.api_key || "")},
      {"CLAUDE_MODEL", to_charlist(cc_config.model || "")},
      {"ANTHROPIC_PROVIDER", to_charlist(cc_config.provider || "")},
      {"ANTHROPIC_ENDPOINT", to_charlist(cc_config.endpoint || "")}
    ]
    ...
    case execute_turn(state, turn_number) do
      {:ok, turn_result} ->
        ...
        if turn_number < state.max_turns do
          ...
          emit_done(new_state)
          stop_claude_code_session(new_state)
      ...
      {:error, reason} ->
        ...
        stop_claude_code_session(state)
      ...
      {^port, {:data, {:eol, line}}} ->
        case Jason.decode(to_string(line)) do
          {:ok, %{"method" => "turn/completed", "params" => params}} ->
            {:ok,
             %{
               session_id: params["sessionId"] || "claude",
               turn_id: turn_id,
               result: params
             }}
          ...
          {:ok, %{"method" => "tool/call", "params" => params}} = payload ->
            ...
            send(self(), {:claude_code_message, payload})
          ...
          {:ok, payload} ->
            send(self(), {:claude_code_message, %{payload: payload}})
            await_turn_completion(port, turn_id)
  ...
  defp process_message(%{payload: payload} = msg, state) do
    ...
    case method do
      ...
      _ ->
        Logger.debug("ClaudeCodeRunner received: #{inspect(method)}")
        state
    end
  end

  defp process_message(msg, state) do
    Logger.debug("ClaudeCodeRunner received unexpected message: #{inspect(msg)}")
    state
  end
  ...
  defp stop_claude_code_session(state) do
  ...
  defp accumulate_token_counts(existing, _turn_result) do
    # Token accumulation will be implemented when Claude Code returns usage data
    existing
  end
