defmodule SymphonyElixir.AgentRunner.HermesRunner do
  @moduledoc """
  AgentRunner implementation for Hermes using the ACP protocol.

  Wraps the Hermes chat session and emits typed events for the orchestrator:
  - `turn_start` / `turn_end` for each Hermes turn
  ...
  def init({workspace_path, prompt, config}) do
    ...
    issue = Map.get(config, :issue, %{id: "runner", identifier: "runner", title: "Hermes Runner"})
  ...
  def handle_continue(:start_session, state) do
    case start_hermes_session(state) do
      {:ok, port, session_id} ->
  ...
  @impl true
  def handle_info({:hermes_message, message}, state) do
  ...
  defp start_hermes_session(state) do
    workspace = state.workspace_path
    hermes_cfg = state.hermes_config

    # Build environment with API keys
    env = build_hermes_env(hermes_cfg)

    # Build command
    cmd = hermes_cfg.command
      ...
      {^port, {:data, {:eol, line}}} ->
        case Jason.decode(to_string(line)) do
          {:ok, %{"method" => "session/started", "params" => params}} ->
            session_id = params["sessionId"] || "hermes-#{:rand.uniform(999_999)}"
          ...
          {:ok, payload} ->
            ...
            session_id = extract_session_id(payload) || "hermes-#{:rand.uniform(999_999)}"
            {:ok, port, session_id}

          {:error, _} ->
            # Non-JSON response might mean ready to proceed
            {:ok, port, "hermes-#{:rand.uniform(999_999)}"}
        end
  ...
  defp build_hermes_env(hermes_cfg) do
    env_list = [
      {"LINEAR_API_KEY", to_charlist(System.get_env("LINEAR_API_KEY") || "")},
      {"HERMES_API_KEY", to_charlist(hermes_cfg.api_key || "")},
      {"HERMES_MODEL", to_charlist(hermes_cfg.model || "")},
      {"HERMES_PROVIDER", to_charlist(hermes_cfg.provider || "")},
      {"HERMES_ENDPOINT", to_charlist(hermes_cfg.endpoint || "")}
    ]
      ...
      {^port, {:data, {:eol, line}}} ->
        case Jason.decode(to_string(line)) do
          {:ok, %{"method" => "turn/completed", "params" => params}} ->
            {:ok,
             %{
               session_id: params["sessionId"] || "hermes",
               turn_id: turn_id,
               result: params
             }}
          ...
          {:ok, %{"method" => "tool/call", "params" => params}} = payload ->
            ...
            send(self(), {:hermes_message, payload})
          ...
          {:ok, payload} ->
            send(self(), {:hermes_message, %{payload: payload}})
            await_turn_completion(port, turn_id)
  ...
  defp process_message(%{payload: payload} = msg, state) do
    ...
    case method do
      ...
      _ ->
        Logger.debug("HermesRunner received: #{inspect(method)}")
        state
    end
  end

  defp process_message(msg, state) do
    Logger.debug("HermesRunner received unexpected message: #{inspect(msg)}")
    state
  end
  ...
  defp stop_hermes_session(state) do
  ...
  defp accumulate_token_counts(existing, _turn_result) do
    # Token accumulation will be implemented when Hermes returns usage data
    existing
  end
  ...
  defp build_continuation_prompt(turn_number, max_turns) do
    ...
    - The previous Hermes turn completed normally, but the Linear issue is still in an active state.
